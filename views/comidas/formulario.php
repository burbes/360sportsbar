...
/*
 * ***********************************************
 * Objetivo:     Formulário de Cadastro de Comidas
 *               
...
<form action="" method="post" name="comidas_form" style="display: none;" enctype="multipart/form-data">

    <fieldset>
        <legend>Dados das Comidas</legend>
        <table class='w100p'>
            <tr>
            <td class='w50p'>
            ...
            <select name="ID_TIPO" id="sltTipos" autofocus="autofocus" class="w100px" >
                <option value=''>&nbsp;</option>
                <?php
                echo Comidas::selectOptionTipos();
                ?>
            </select>
            ...
            <div id="divImg" onmouseover="objComidas.fnShowImage(true, $('#hddImg' + $('#hddID').val()).val());" onmouseout="objComidas.fnShowImage(false)" class="cp "></div>
    ...
    <input type="hidden" name="ID_COMIDA" id="hddID" value="" />
    ...
    <!-- BOTÃO SUBMIT -->
    <input type="button" id="btnSalvar" value="Salvar" onclick="objComidas.fnSubmitForm();" />

    <!-- BOTÃO LIMPAR -->
    <input type="reset" id="btnLimpar" value="Limpar" onclick="objComidas.fnResetForm()" />
